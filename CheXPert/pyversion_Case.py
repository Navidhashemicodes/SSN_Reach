...
def generate_data_chunk(N, N_perturbed, indices, de, Inputs, model, device):
...
def CheXpert_exp(start_loc, N_perturbed, delta_rgb, image_name, Nt, N_dir, Ns, Nsp, rank, guarantee, device):
    ...
    Y, X = generate_data_chunk(Nt, N_perturbed, indices, de, Inputs, UnetFunc, device)
    ...
    for nc, curr_len in enumerate(chunk_sizes):
        ...
        Y_test_nc, X_test_nc = generate_data_chunk(curr_len, N_perturbed, indices, de, Inputs, UnetFunc, device)
    ...
    save_name = f"CI_result_middle_guarantee_ReLU_relaxed_eps_{delta_rgb}_Npertubed_{N_perturbed}_{base_name}.pt"
...
# CheXpert_exp(start_loc, N_perturbed, delta_rgb, image_name, Nt, N_dir, Ns, Nsp, rank, guarantee, device)