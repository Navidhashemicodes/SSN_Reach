...
def main(mat_file_path: str, num_files: int, N_dir: int):
    ...
    Y = {}
    new_shape = (720 * 960 * 12, N_dir // num_files)
    ...
    # scipy.io.savemat('directions.mat', {'Directions': A_list  , 'Direction_Training_time' : Training_time})


    # Convert principal directions to a single NumPy array
    A_list_np = np.hstack([A.to('cpu').numpy().astype(np.float32) for A in A_list])

    # Save using NumPy's .npz format (can handle large arrays)
    np.savez('directions.npz', Directions=A_list_np, Direction_Training_time=Training_time)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mat_file_path', type=str, required=True,
                        help='Path to the input .mat file.')
    parser.add_argument('--num_files', type=int, required=True,
                        help='Number of .mat file variables (e.g., Y1, Y2, ..., Yn).')
    parser.add_argument('--N_dir', type=int, required=True,
                        help='Total number of data instances (will be divided across num_files).')
    args = parser.parse_args()

    main(args.mat_file_path, args.num_files, args.N_dir)

# # Define the path to the .mat file
# mat_file_path = r"/home/hashemn/Documents/Navid/CamVid_project_de_3_255/BiSeNet_Reach_eps_1/Direction_data.mat"
# 
# num_files = 5
# instances = 150//num_files
# # Load MATLAB data
# mat_data = scipy.io.loadmat(mat_file_path)  
# 
# # Extract individual arrays
# Y = {}
# new_shape = (720 * 960 * 12, instances)
# for i in range(1,num_files+1):
#     key = f'Y{i}'
#     Y[key] = torch.tensor(mat_data[key], dtype=torch.float32).reshape(new_shape)
# 
