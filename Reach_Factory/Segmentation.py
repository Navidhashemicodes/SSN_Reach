...
class Segmentor:
    ...
    def is_classified(self):
        ...
        H_lb = -Conf.unsqueeze(1)
        H_ub = -H_lb

            
        if mode == 'ReLU':
            center, generators , pred_lb, pred_ub, reachability_time = self.Reach_ReLU()
            pred_ub = pred_ub.squeeze()
            pred_lb = pred_lb.squeeze()
            
        if mode == 'Linear':
            Map = Data['Map']
            center, generators , pred_lb, pred_ub, reachability_time = self.Reach_Linear(Map)
            
        verif_start = time()    
        S_center = Directions.T @ center + C.unsqueeze(1)
        S_d = S_center[label] - S_center
        SR = Directions.T @ generators
        SR_d = SR[label] - SR
        SR_d_pos = 0.5 * (SR_d + SR_d.abs())
        SR_d_neg = 0.5 * (SR_d - SR_d.abs())
        I = torch.eye(n_class).to(self.device)
        I_d = I[label] - I
        I_d_pos = 0.5 * (I_d + I_d.abs())
        I_d_neg = 0.5 * (I_d - I_d.abs())
        Lb = S_d + SR_d_pos @ pred_lb.unsqueeze(1) + I_d_pos @ H_lb + SR_d_neg @ pred_ub.unsqueeze(1) + I_d_neg @ H_ub
        ...
        verif_time = time() - verif_start
        
        verification_runtime = Data['train_data_run_1'] + Data['trn_time1'] + sum(Data['test_data_run']) + \
                               sum(Data['res_test_time']) + Data['conformal_time'] + reachability_time + \
                                Data['Direction_Training_time'] + Data['Model_training_time'] + verif_time
                               
        print(f'the Decision was {Decision} and the runtime was {verification_runtime} seconds')
        return Decision, verification_runtime
